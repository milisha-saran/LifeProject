...
import { type Chore, type ChoreFormData, FREQUENCY_OPTIONS, STATUS_OPTIONS } from '@/types/recurring';
...
export function ChoreForm({ chore, open, onOpenChange, onSubmit, isLoading }: ChoreFormProps) {
  ...
  const {
    ...
    defaultValues: chore ? {
      ...
      start_time: chore.start_time ? new Date(chore.start_time) : undefined,
      end_time: chore.end_time ? new Date(chore.end_time) : undefined,
      eta_hours: chore.eta_hours,
      status: chore.status,
      frequency_type: chore.frequency_type,
      frequency_value: chore.frequency_value,
      next_due_date: new Date(chore.next_due_date),
    } : {
      status: 'Not Started',
  ...
  const handleFormSubmit = async (data: ChoreFormData) => {
    try {
      await onSubmit(data);
      reset();
      onOpenChange(false);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{chore ? 'Edit Chore' : 'Create New Chore'}</DialogTitle>
        </DialogHeader>
